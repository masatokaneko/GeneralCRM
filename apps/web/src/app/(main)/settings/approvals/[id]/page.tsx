...
export default function EditApprovalProcessPage() {
  const router = useRouter();
  const params = useParams();
  const processId = params.id as string;

  const { data: process, isLoading: isLoadingProcess } =
    useApprovalProcess(processId);
  ...
  useEffect(() => {
    if (process) {
      ...
      setFilterLogic(process.filterLogic || "");
      setSteps(process.steps);
      setSubmitActions(process.actions.onSubmit);
      setApproveActions(process.actions.onApprove);
      setRejectActions(process.actions.onReject);
    }
  }, [process]);
  ...
  if (isLoadingProcess) {
    return (
      <div className="flex items-center justify-center py-12">
        <p className="text-muted-foreground">Loading approval process...</p>
      </div>
    );
  }
  ...
  if (!process) {
    return (
      <div className="flex flex-col items-center justify-center py-12">
        <p className="text-muted-foreground">Approval process not found</p>
        <Button variant="outline" className="mt-4" asChild>
          <Link href="/settings/approvals">Back to Approval Processes</Link>
        </Button>
      </div>
    );
  }
    ...
    <div className="space-y-6">
      <div className="flex items-center gap-4">
        ...
        <div className="flex-1">
          <h1 className="text-2xl font-bold">Edit Approval Process</h1>
          <p className="text-muted-foreground">
            Modify the approval process settings and steps
          </p>
        </div>
        ...
        <Button
          onClick={handleSave}
          disabled={!isValid || updateProcess.isPending}
        >
                ...
                <div className="grid gap-2">
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="font-medium">Locked (Recommended)</span>
                    </div>
